...
public class RhymingDict {
	...
	public static void storeRhyme(DictionaryInterface rhymingDict, String line) {
		...
		Object result = rhymingDict.get(rhymeGroup);

		if (result == null){
			MySortedLinkedList spot = new MySortedLinkedList();
			spot.add(word);
			rhymingDict.put(rhymeGroup, spot);

		} else {

			((MySortedLinkedList) result).add(word);
		}
	}
	...
	public static void removeUnrhymables(DictionaryInterface rhymingDict) {
		...
		for (int i=0; i < tempKeys.length; i++){
			MySortedLinkedList list = (MySortedLinkedList) rhymingDict.get(tempKeys[i]);
			
			if (list.size() == 1){
				rhymingDict.remove(tempKeys[i]);
			}
		}
	} 
